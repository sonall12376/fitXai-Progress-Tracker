...
export const defaultForm: ProgressFormState = {
  workoutCompleted: false, workoutType: 'Strength Training',
  workoutDuration: '', caloriesBurned: '', caloriesConsumed: '',
  steps: '', sleepHours: '', waterIntake: '',
  mood: 'Good', energyLevel: 5,
  hasInjury: false, painLevel: 0, injuryDetails: '', notes: '',
};

const WORKOUT_TYPES = ['Strength Training','Cardio','HIIT','Yoga','Chest & Triceps','Back & Biceps','Legs','Shoulders','Full Body','Running','Cycling'];
const MOODS = ['Energetic','Good','Tired','Exhausted','Stressed'];
const MOOD_EMOJI: Record<string,string> = { Energetic:'⚡', Good:'😊', Tired:'😴', Exhausted:'🥱', Stressed:'😤' };
...
function Label({ t, req }: { t:string; req?:boolean }) {
  return <Text style={f.label}>{t}{req ? <Text style={{ color:C.red }}> *</Text> : ''}</Text>;
}

function NumIn({ val, set, ph, unit, dec }: { val:string; set:(v:string)=>void; ph:string; unit?:string; dec?:boolean }) {
  return (
    <View style={f.inputRow}>
      <TextInput style={f.input} value={val} onChangeText={set} placeholder={ph} placeholderTextColor={C.text2} keyboardType={dec?'decimal-pad':'numeric'} returnKeyType="done" />
      {unit ? <Text style={f.unit}>{unit}</Text> : null}
    </View>
  );
}

function Chips({ opts, sel, onSel, emoji }: { opts:string[]; sel:string; onSel:(v:string)=>void; emoji?:Record<string,string> }) {
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
      <View style={{ flexDirection:'row', gap:8, paddingVertical:4 }}>
        {opts.map(o => {
          const active = o === sel;
          return (
            <TouchableOpacity key={o} onPress={() => onSel(o)} style={[f.chip, active && f.chipActive]} activeOpacity={0.8}>
              {active
                ? <LinearGradient colors={[C.purple, C.pink]} start={{x:0,y:0}} end={{x:1,y:0}} style={f.chipGrad}><Text style={f.chipTxtOn}>{emoji?.[o]??''} {o}</Text></LinearGradient>
...
function Dots({ val, min, max, set, color=C.purple }: { val:number; min:number; max:number; set:(v:number)=>void; color?:string }) {
      ...
      <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', gap:2 }}>
        {steps.map(n => (
          <TouchableOpacity key={n} onPress={() => set(n)}
            style={[f.dot, { backgroundColor: n<=val ? color : C.card2, width: n===val?28:18, height: n===val?28:18 }]}>
...
// ── Main Form ─────────────────────────────────────────────────
export function ProgressForm({ onSubmit, submitting }: { onSubmit:(f:ProgressFormState)=>void; submitting:boolean }) {
  const [form, setForm] = useState<ProgressFormState>(defaultForm);
  const upd = <K extends keyof ProgressFormState>(k:K, v:ProgressFormState[K]) => setForm(p => ({...p,[k]:v}));
    ...
    <View style={{ gap:10 }}>
      ...
      <Block title="🏋️  Workout">
        ...
        <View style={f.switchRow}>
          <Switch value={form.workoutCompleted} onValueChange={v => upd('workoutCompleted',v)}
            trackColor={{ false:C.card2, true:C.purple }} thumbColor="#fff" />
      ...
      <Block title="🩹  Injury & Pain">
        ...
        <View style={f.switchRow}>
          <Switch value={form.hasInjury} onValueChange={v => upd('hasInjury',v)}
            trackColor={{ false:C.card2, true:C.red }} thumbColor="#fff" />
        ...
        {form.hasInjury && <>
          ...
          <TextInput style={f.area} value={form.injuryDetails} onChangeText={v => upd('injuryDetails',v)}
            placeholder="Describe location, movement restrictions..." placeholderTextColor={C.text2}
      ...
      <Block title="📝  Additional Notes">
        <TextInput style={f.area} value={form.notes} onChangeText={v => upd('notes',v)}
          placeholder="How did today feel? Any soreness, motivation, or observations..." placeholderTextColor={C.text2}
          multiline numberOfLines={4} textAlignVertical="top" />
      </Block>

      {/* ── Submit ── */}
      <TouchableOpacity onPress={() => onSubmit(form)} disabled={submitting} activeOpacity={0.85} style={f.submitWrap}>
        <LinearGradient colors={submitting ? [C.card2,C.card2] : [C.purple,C.pink]} start={{x:0,y:0}} end={{x:1,y:0}} style={f.submitGrad}>
          {submitting
            ? <View style={{ flexDirection:'row', gap:10, alignItems:'center' }}><ActivityIndicator color={C.text2} size="small" /><Text style={[f.submitTxt, { color:C.text2 }]}>Generating AI Analysis...</Text></View>
