...
export const DashboardLayout = () => {
  ...
  const { data, isOffline, isLoading } = useDataRouting(timeRange);
    ...
    <div className="max-w-[1120px] mx-auto w-full p-4 md:p-6 lg:p-8 relative min-h-screen pb-24">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {isLoading && !isOffline && <span className="text-purple text-xs font-bold block">Refreshing data...</span>}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4 col-span-1 md:col-span-2 lg:col-span-3">
          <MiniCard title="Weight Progress" tag={`↓ ${(Math.abs(data.weightProgress[0]?.weight - data.weightProgress[data.weightProgress.length - 1]?.weight) || 0).toFixed(1)} kg`}>
            <div className="text-[19px] font-extrabold my-1">{data.weightProgress[data.weightProgress.length - 1]?.weight.toFixed(1)} <small className="text-[12px] text-textSec">kg</small></div>
          ...
          <MiniCard title="Calories Burned">
            <div className="text-[10.5px] text-textSec mb-2">Avg {Math.round(data.calorieHistory.reduce((sum, item) => sum + item.calories, 0) / Math.max(data.calorieHistory.length, 1))} kcal</div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
        <HexAchievement number={data.progressScore.workoutsCompleted.toString()} title="Workouts" gradient="linear-gradient(160deg, #FFD60A, #CA8A04)" />
        <HexAchievement number={`${(Math.abs(data.weightProgress[0]?.weight - data.weightProgress[data.weightProgress.length - 1]?.weight) || 0).toFixed(1)}KG`} title="Lost" gradient="linear-gradient(160deg, #FB923C, #C2410C)" />
        <HexAchievement number={data.progressScore.streakDays.toString()} title="Day Streak" gradient="linear-gradient(160deg, #F97316, #B91C1C)" />
      </div>
